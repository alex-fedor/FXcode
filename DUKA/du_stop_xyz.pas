program find_qV_stop_XYZ; {for BuyStop and SellStop}
uses crt,strings;
{const spread=0.0003;}
       {z_max=1970;}
var
...
Day,S,week_day,Date,PositionX,PositionX_ASK,week_day_ini,Profit_ini,Loss_ini,calendar,history_0,history_1,z_min_ini,z_max_ini,p_min_ini,p_max_ini,l_min_ini,l_max_ini,spread_ini,tau_0_ini,week:string;
...
assign(f3,'d' + week_day_ini + '_t'+ tau_0_ini + '_stop_xyz_{' + week + '}.csv');
rewrite(f3);
...
{******************      BuyStop      ************************************************************}
if ({MaxP[i] + spread}MaxP_ASK[i] >= BuyStop_Open) and (BuyStop_Active=0) and (SellStop_Active=0) then
begin
BuyStop_Active:=1;
...
begin
if ({MinP[i] + spread}MinP_ASK[i] >= BuyStop_Open) then BuyStop_Floating_PL:=MaxP[i]{ + spread} - BuyStop_Open;{GOOD!}
if ({MaxP[i] + spread}MaxP_ASK[i] <  BuyStop_Open) then BuyStop_Floating_PL:=MinP[i]{ + spread} - BuyStop_Open;{GOOD!}
if ({MinP[i] + spread}MinP_ASK[i] <  BuyStop_Open) then BuyStop_Floating_PL:=MinP[i]{ + spread} - BuyStop_Open;{THINKING...}
...
begin
if (MaxP[i] <= SellStop_Open) then SellStop_Floating_PL:=SellStop_Open - {MinP[i] - spread}MinP_ASK[i];{GOOD!}
if (MinP[i] >  SellStop_Open) then SellStop_Floating_PL:=SellStop_Open - {MaxP[i] - spread}MaxP_ASK[i];{GOOD!}
if (MaxP[i] >  SellStop_Open) then SellStop_Floating_PL:=SellStop_Open - {MaxP[i] - spread}MaxP_ASK[i];{THINKING...}
...
Operation_type_R:='BUY';
str(({BuyStop_Loss-BuyStop_Open}-Profit)*10000:3:1,BuyStop_Loss_R);
Daily_profit:=({BuyStop_Loss-BuyStop_Open}-Profit)*10000;
...
str(BuyStop_Loss_Time,BuyStop_Loss_Time_R);
Operation_type_R:='BUY';
str(({BuyStop_Floating_PL}-Profit)*10000:3:1,BuyStop_Loss_R);
Daily_profit:=({BuyStop_Floating_PL}-Profit)*10000;
Report:=Concat(Report,',',Operation_type_R,',',BuyStop_Active_Time_R,',',BuyStop_Loss_Time_R,',',BuyStop_Loss_R);
{writeln(Report);}
end;
...
Operation_type_R:='SELL';
str(({SellStop_Open-SellStop_Loss}-Profit)*10000:3:1,SellStop_Loss_R);
Daily_Profit:=({SellStop_Open-SellStop_Loss}-Profit)*10000;
...
str(SellStop_Loss_Time,SellStop_Loss_Time_R);
Operation_type_R:='SELL';
str(({SellStop_Floating_PL}-Profit)*10000:3:1,SellStop_Loss_R);
Daily_profit:=({SellStop_Floating_PL}-Profit)*10000;
Report:=Concat(Report,',',Operation_type_R,',',SellStop_Active_Time_R,',',SellStop_Loss_Time_R,',',SellStop_Loss_R);
{writeln(Report);}
end;
...
writeln(f3,Profit*10000:5:1,' ',Loss*10000:5:1,' ',Volatility:9:5,'  ',Total_Profit/(Profit*10000*53):9:3{,' || ',p/p_max*100:7:2,' ',l/l_max*100:7:2,' ',z/z_max*100:7:2});
