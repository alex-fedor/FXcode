...
Day,S,week_day,Date,PositionX,PositionX_ASK,week_day_ini,Profit_ini,Loss_ini,calendar,history_0,history_1,z_min_ini,z_max_ini,p_min_ini,p_max_ini,l_min_ini,l_max_ini,spread_ini,tau_0_ini,week,TPCL_ini:string;
OpenP,MaxP,MinP,CloseP,VolumeP,OpenP_ASK,MaxP_ASK,MinP_ASK,CloseP_ASK,VolumeP_ASK:array[0..1439]of real;

 BuyLimit_Open, BuyLimit_Profit, BuyLimit_Loss,TPCL:real;
...
q_Volat,Volatility,Profit,Loss,spread,Loss_min,Loss_interval:real;

BuyLimit_Floating_PL,SellLimit_Floating_PL:real;

f,f2,f3,f_ini,f_ASK:text;

Total_Profit,Daily_Profit,Total_Profit_current_Loss:real;

BEGIN
{find day and creation of quotes DB}
clrscr;
assign(f_ini,'ini_xyz');
reset(f_ini);
...
readln(f_ini,history_0);            history_0:=Copy( history_0,14,Length( history_0));
readln(f_ini,history_1);            history_1:=Copy( history_1,14,Length( history_1));
...
readln(f_ini,TPCL_ini);            TPCL_ini:=Copy(TPCL_ini,14,Length(TPCL_ini));       val(TPCL_ini,TPCL,code);
close(f_ini);

assign(f2,calendar);{assign(f2,'calendar_20110221-20120217.csv');}
assign(f,history_0);{assign(f,'EURUSD_M1_20110221-20120217.csv');}
assign(f_ASK,history_1);{assign(f,'EURUSD_M1_20110221-20120217.csv');}
...
Loss:=l_max*0.00001;
Loss_min:=Loss;
Loss_interval:=Loss-l_min*0.00001;
repeat
begin
...
repeat
k:=k+1;
readln(f,PositionX); readln(f_ASK,PositionX_ASK);
Date:=Copy(PositionX,1,10);
if (Date=S) then
begin
find:=1;
i:=i+1;
val(PositionX[18..24],OpenP[i],code);                   val(PositionX_ASK[18..24],OpenP_ASK[i],code);
val(PositionX[26..32],MaxP[i],code);                    val(PositionX_ASK[26..32],MaxP_ASK[i],code);
val(PositionX[34..40],MinP[i],code);                    val(PositionX_ASK[34..40],MinP_ASK[i],code);
val(PositionX[42..48],CloseP[i],code);                  val(PositionX_ASK[42..48],CloseP_ASK[i],code); 
val(PositionX[50..Length(PositionX)],VolumeP[i],code);  val(PositionX_ASK[50..Length(PositionX_ASK)],VolumeP_ASK[i],code);
{writeln(i,' ',PositionX);}
end;
...
{******************      BuyLimit      ************************************************************}
if ({MinP[i] + spread}MinP_ASK[i] <= BuyLimit_Open) and (BuyLimit_Active=0) and (SellLimit_Active=0) then
begin
BuyLimit_Active:=1;
...
begin
if ({MinP[i] + spread}MinP_ASK[i] >= BuyLimit_Open) then BuyLimit_Floating_PL:=MaxP[i] - BuyLimit_Open;{GOOD!}
if ({MaxP[i] + spread}MaxP_ASK[i] <  BuyLimit_Open) then BuyLimit_Floating_PL:=MinP[i] - BuyLimit_Open;{GOOD!}
if ({MinP[i] + spread}MinP_ASK[i] <  BuyLimit_Open) then BuyLimit_Floating_PL:=MinP[i] - BuyLimit_Open;{THINKING...}
...
begin
if (MaxP[i] <= SellLimit_Open) then SellLimit_Floating_PL:=SellLimit_Open - {MinP[i] - spread}MinP_ASK[i];{GOOD!}
if (MinP[i] >  SellLimit_Open) then SellLimit_Floating_PL:=SellLimit_Open - {MaxP[i] - spread}MaxP_ASK[i];{GOOD!}
if (MaxP[i] >  SellLimit_Open) then SellLimit_Floating_PL:=SellLimit_Open - {MaxP[i] - spread}MaxP_ASK[i];{THINKING...}
...
Operation_type_R:='BUY';
str(({BuyLimit_Loss-BuyLimit_Open}-Profit)*10000:3:1,BuyLimit_Loss_R);
Daily_profit:=({BuyLimit_Loss-BuyLimit_Open}-Profit)*10000;
...
str(BuyLimit_Loss_Time,BuyLimit_Loss_Time_R);
Operation_type_R:='BUY';
str(({BuyLimit_Floating_PL}-Profit)*10000:3:1,BuyLimit_Loss_R);
Daily_profit:=({BuyLimit_Floating_PL}-Profit)*10000;
Report:=Concat(Report,',',Operation_type_R,',',BuyLimit_Active_Time_R,',',BuyLimit_Loss_Time_R,',',BuyLimit_Loss_R);
{writeln(Report);}
end;
...
Operation_type_R:='SELL';
str(({SellLimit_Open-SellLimit_Loss}-Profit)*10000:3:1,SellLimit_Loss_R);
Daily_Profit:=({SellLimit_Open-SellLimit_Loss}-Profit)*10000;
...
str(SellLimit_Loss_Time,SellLimit_Loss_Time_R);
Operation_type_R:='SELL';
str(({SellLimit_Floating_PL}-Profit)*10000:3:1,SellLimit_Loss_R);
Daily_Profit:=({SellLimit_Floating_PL}-Profit)*10000;
Report:=Concat(Report,',',Operation_type_R,',',SellLimit_Active_Time_R,',',SellLimit_Loss_Time_R,',',SellLimit_Loss_R);
{writeln(Report);}
end;
...
Total_Profit_current_Loss:=Total_Profit/(Profit*10000*53);
end;{week_day}
...
if (Total_Profit_current_Loss>=TPCL) and (Loss<Loss_min) then
begin
Loss_interval:=Loss_min-Loss;
Loss_min:=Loss;
end;
Loss:=Loss_min-(Loss_interval/2);
if (Total_Profit_current_Loss< TPCL) and (Loss>=Loss_min) then 
begin
Loss_interval:=Loss-Loss_min;
Loss:=Loss_min+(Loss_interval/2);
end;
if (Total_Profit_current_Loss< TPCL) and (Loss<Loss_min) then 
begin
...
{l:=l+1;}
until {(keypressed) or }(Loss_interval<=0.00001);
writeln(f3,Profit*10000:5:1,' ',Loss_min*10000:5:1,' ',Volatility:9:5);
