...
Day,S,week_day,Date,PositionX,week_day_ini,Profit_ini,Loss_ini,calendar,history,z_min_ini,z_max_ini,p_min_ini,p_max_ini,l_min_ini,l_max_ini,spread_ini,tau_0_ini,tau_X_ini,week,TPCL_ini:string;
OpenP,MaxP,MinP,CloseP,VolumeP:array[0..1439]of real;

 BuyStop_Open, BuyStop_Profit, BuyStop_Loss,TPCL:real;
...
assign(f3,'d' + week_day_ini + 'S_FindLoss' + '_['+ tau_0_ini +'^' + tau_X_ini +']' + '_{' + week + '}.csv');
rewrite(f3);
...
begin
if (MinP[i] + spread >= BuyStop_Open) then BuyStop_Floating_PL:=MaxP[i]{ + spread} - BuyStop_Open;{GOOD!}
if (MaxP[i] + spread <  BuyStop_Open) then BuyStop_Floating_PL:=MinP[i]{ + spread} - BuyStop_Open;{GOOD!}
if (MinP[i] + spread <  BuyStop_Open) then BuyStop_Floating_PL:=MinP[i]{ + spread} - BuyStop_Open;{THINKING...}
...
begin
if (MaxP[i] <= SellStop_Open) then SellStop_Floating_PL:=SellStop_Open - MinP[i] - spread;{GOOD!}
if (MinP[i] >  SellStop_Open) then SellStop_Floating_PL:=SellStop_Open - MaxP[i] - spread;{GOOD!}
if (MaxP[i] >  SellStop_Open) then SellStop_Floating_PL:=SellStop_Open - MaxP[i] - spread;{THINKING...}
