program candles; {for BuyStop and SellStop}
uses crt,strings;
const {spread=0.0003;}
      z_max=3500;
var
...
OpenP,MaxP,MinP,CloseP,VolumeP:array[0..1439]of real;
 ...
 BuyStop_Open, BuyStop_Profit, BuyStop_Loss,MaxP_day,MinP_day,Day_Volatility,Body,Upper,Lower,ABS_Body:real;
SellStop_Open,SellStop_Profit,SellStop_Loss:real;

found_entries,missed_entries,volat_daily,OpenP_R,MaxP_day_R,MinP_day_R,CloseP_R,Day_Volatility_R,Body_R,Upper_R,Lower_R,ABS_Body_R:string;
...
assign(f3,'d' + week_day_ini + {'_t'+ tau_0_ini + }'______candles_info_{' + week + '}.csv');
rewrite(f3);
writeln(f3,'Date,Total,Missed,Open,Max,Min,Close,Body,Volatility,ABS(Body),Upper,Lower');
...
begin
close(f);
Report:=Concat(S,',0,0,0,0,0,0,0,0');
{writeln(Report);}
end;
...
if ((END_of_time)<  10)                          then found_entries:='   '+found_entries;
if ((END_of_time)>= 10) and ((END_of_time)< 100) then found_entries:='  '+found_entries;
if ((END_of_time)>=100) and ((END_of_time)<1000) then found_entries:=' '+found_entries;
...
MaxP_day:=MaxP[0];
MinP_day:=MinP[0];
for i:=1 to END_of_time do
begin
if MaxP[i]>=MaxP_day then MaxP_day:=MaxP[i];
if MinP[i]<=MinP_day then MinP_day:=MinP[i];
end;
i:=i+1;
Day_Volatility:=(MaxP_day-MinP_day)*10000;
Body:=(CloseP[END_of_time]-OpenP[0])*10000;
if Body>=0 then
begin
Upper:=(MaxP_day-CloseP[END_of_time])*10000;
Lower:=(OpenP[0]-MinP_day)*10000;
end;
if Body< 0 then
begin
Upper:=(MaxP_day-OpenP[0])*10000;
Lower:=(CloseP[END_of_time]-MinP_day)*10000;
end;
ABS_Body:=abs(Body);
str(OpenP[0]:5:5,OpenP_R);
str(MaxP_day:5:5,MaxP_day_R);
str(MinP_day:5:5,MinP_day_R);
str(CloseP[END_of_time]:5:5,CloseP_R);
str(Body:6:1,Body_R);
str(Day_Volatility:5:1,Day_Volatility_R);
str(ABS_Body:5:1,ABS_Body_R);
str(Upper:5:1,Upper_R);
str(Lower:5:1,Lower_R);
Report:=Concat(Report,',',OpenP_R,',',MaxP_day_R,',',MinP_day_R,',',CloseP_R,',',Body_R,',',Day_Volatility_R,',',ABS_Body_R,',',Upper_R,',',Lower_R);
