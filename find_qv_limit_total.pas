program find_qV_limit_total; {for BuyLimit and SellLimit}
uses crt,strings;
{const spread=0.0003;}
       {z_max=1970;}
var
...
Day,S,week_day,Date,PositionX,week_day_ini,Profit_ini,Loss_ini,calendar,history,z_min_ini,z_max_ini,p_min_ini,p_max_ini,l_min_ini,l_max_ini,spread_ini:string;
...
assign(f3,'PLqV_limit_total_' + week_day_ini + '.csv');
rewrite(f3);
...
BuyLimit_Open:=OpenP[0] - q_Volat + spread;
BuyLimit_Profit:=BuyLimit_Open + Profit;
BuyLimit_Loss:=  BuyLimit_Open - {Profit * Coef}Loss;

SellLimit_Open:=OpenP[0] + q_Volat;
SellLimit_Profit:=SellLimit_Open - Profit;
SellLimit_Loss:=  SellLimit_Open + {Profit * Coef}Loss;
